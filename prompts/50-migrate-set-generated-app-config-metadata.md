# Prompt: Migrate SetGeneratedAppConfigMetadata for Multithreading
...
## What This Task Does
Sets metadata on generated app.config items. May involve path metadata.
...
- **Attribute check**: `typeof(SetGeneratedAppConfigMetadata).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(SetGeneratedAppConfigMetadata).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
