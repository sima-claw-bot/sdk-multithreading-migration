# Prompt: Migrate CheckForTargetInAssetsFile for Multithreading
...
## What This Task Does
Verifies the target framework exists in the assets/lock file.
...
- **Attribute check**: `typeof(CheckForTargetInAssetsFile).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(CheckForTargetInAssetsFile).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
