# Prompt: Migrate RemoveDuplicatePackageReferences for Multithreading
...
## Task File
`src/Tasks/Microsoft.NET.Build.Tasks/RemoveDuplicatePackageReferences.cs`

## What This Task Does
Removes duplicate PackageReference items, keeping the one with the highest version.
...
- **Attribute check**: `typeof(RemoveDuplicatePackageReferences).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(RemoveDuplicatePackageReferences).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
