# Prompt: Migrate ResolveTargetingPackAssets for Multithreading
...
## What This Task Does
Resolves assets from targeting packs. May involve path resolution for pack directories.
...
- **Attribute check**: `typeof(ResolveTargetingPackAssets).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(ResolveTargetingPackAssets).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
