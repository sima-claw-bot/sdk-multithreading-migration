# Prompt: Migrate CollatePackageDownloads for Multithreading
...
## Task File
`src/Tasks/Microsoft.NET.Build.Tasks/CollatePackageDownloads.cs`

## What This Task Does
Collates package downloads from multiple target frameworks into a unified list.
...
- **Attribute check**: `typeof(CollatePackageDownloads).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(CollatePackageDownloads).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
