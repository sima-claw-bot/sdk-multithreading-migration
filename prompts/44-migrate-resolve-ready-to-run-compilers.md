# Prompt: Migrate ResolveReadyToRunCompilers for Multithreading
...
## What This Task Does
Resolves the ReadyToRun compiler (crossgen/crossgen2) paths. Likely uses File.Exists for tool verification.
...
- **Attribute check**: `typeof(ResolveReadyToRunCompilers).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(ResolveReadyToRunCompilers).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
