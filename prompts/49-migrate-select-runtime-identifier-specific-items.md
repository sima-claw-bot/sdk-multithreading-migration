# Prompt: Migrate SelectRuntimeIdentifierSpecificItems for Multithreading
...
## What This Task Does
Selects items specific to the current RuntimeIdentifier from a broader set.
...
- **Attribute check**: `typeof(SelectRuntimeIdentifierSpecificItems).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(SelectRuntimeIdentifierSpecificItems).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
