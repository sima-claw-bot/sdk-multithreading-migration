# Prompt: Migrate ValidateExecutableReferences for Multithreading
...
## What This Task Does
Validates that executable project references have compatible self-contained settings. Uses BuildEngine6.GetGlobalProperties().
...
- **Attribute check**: `typeof(ValidateExecutableReferences).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(ValidateExecutableReferences).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
