# Prompt: Migrate ResolveCopyLocalAssets for Multithreading
...
## Task File
`src/Tasks/Microsoft.NET.Build.Tasks/ResolveCopyLocalAssets.cs`

## What This Task Does
Resolves which assets should be copied to the output directory.
...
- **Attribute check**: `typeof(ResolveCopyLocalAssets).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(ResolveCopyLocalAssets).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
