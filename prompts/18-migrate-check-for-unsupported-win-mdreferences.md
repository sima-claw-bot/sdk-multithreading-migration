# Prompt: Migrate CheckForUnsupportedWinMDReferences for Multithreading
...
## What This Task Does
Checks for unsupported WinMD references and reports errors.
...
- **Attribute check**: `typeof(CheckForUnsupportedWinMDReferences).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(CheckForUnsupportedWinMDReferences).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
