# Prompt: Migrate CheckForImplicitPackageReferenceOverrides for Multithreading
...
## What This Task Does
Checks if explicit package references override implicit ones and warns.
...
- **Attribute check**: `typeof(CheckForImplicitPackageReferenceOverrides).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(CheckForImplicitPackageReferenceOverrides).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
