# Prompt: Migrate RunCsWinRTGenerator for Multithreading
...
## What This Task Does
Runs the C#/WinRT code generator. Likely extends ToolTask and spawns external process.
...
- **Attribute check**: `typeof(RunCsWinRTGenerator).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(RunCsWinRTGenerator).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
