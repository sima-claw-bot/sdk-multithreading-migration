# Prompt: Migrate GetPublishItemsOutputGroupOutputs for Multithreading
...
## Task File
`src/Tasks/Microsoft.NET.Build.Tasks/GetPublishItemsOutputGroupOutputs.cs`

## What This Task Does
Gets publish items for output group. May involve path operations.
...
- **Attribute check**: `typeof(GetPublishItemsOutputGroupOutputs).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(GetPublishItemsOutputGroupOutputs).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
