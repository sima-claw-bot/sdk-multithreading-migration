# Prompt: Migrate ResolveRuntimePackAssets for Multithreading
...
## What This Task Does
Resolves assets from runtime packs. May involve path resolution for pack directories.
...
- **Attribute check**: `typeof(ResolveRuntimePackAssets).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(ResolveRuntimePackAssets).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
