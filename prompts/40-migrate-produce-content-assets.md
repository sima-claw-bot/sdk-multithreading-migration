# Prompt: Migrate ProduceContentAssets for Multithreading
...
## Task File
`src/Tasks/Microsoft.NET.Build.Tasks/ProduceContentAssets.cs`

## What This Task Does
Produces content assets from NuGet packages. Likely involves file I/O for preprocessing content files.
...
- **Attribute check**: `typeof(ProduceContentAssets).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(ProduceContentAssets).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
