# Prompt: Migrate GenerateShims for Multithreading
...
## What This Task Does
Generates shim executables for .NET tools. Likely involves file I/O for shim creation.
...
- **Attribute check**: `typeof(GenerateShims).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(GenerateShims).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
