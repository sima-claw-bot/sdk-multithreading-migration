# Prompt: Migrate ResolveFrameworkReferences for Multithreading
...
## What This Task Does
Resolves FrameworkReference items to actual framework paths. May use Directory.Exists for pack paths.
...
- **Attribute check**: `typeof(ResolveFrameworkReferences).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(ResolveFrameworkReferences).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
