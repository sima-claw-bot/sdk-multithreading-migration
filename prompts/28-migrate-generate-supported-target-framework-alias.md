# Prompt: Migrate GenerateSupportedTargetFrameworkAlias for Multithreading
...
## Task File
`src/Tasks/Microsoft.NET.Build.Tasks/GenerateSupportedTargetFrameworkAlias.cs`

## What This Task Does
Generates supported target framework alias items from input data.
...
- **Attribute check**: `typeof(GenerateSupportedTargetFrameworkAlias).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(GenerateSupportedTargetFrameworkAlias).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
