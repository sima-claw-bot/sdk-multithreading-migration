# Prompt: Migrate CreateWindowsSdkKnownFrameworkReferences for Multithreading
...
## What This Task Does
Creates KnownFrameworkReference items for Windows SDK targeting packs.
...
- **Attribute check**: `typeof(CreateWindowsSdkKnownFrameworkReferences).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(CreateWindowsSdkKnownFrameworkReferences).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
