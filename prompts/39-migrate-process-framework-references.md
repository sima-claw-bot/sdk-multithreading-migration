# Prompt: Migrate ProcessFrameworkReferences for Multithreading
...
## What This Task Does
Processes FrameworkReference items to resolve targeting packs, runtime packs, and app host packs. Large and complex task.
...
- **Attribute check**: `typeof(ProcessFrameworkReferences).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(ProcessFrameworkReferences).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
