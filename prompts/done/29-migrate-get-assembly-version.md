# Prompt: Migrate GetAssemblyVersion for Multithreading
...
## What This Task Does
Gets the assembly version from an assembly file. May use file I/O to read assembly metadata.
...
- **Attribute check**: `typeof(GetAssemblyVersion).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(GetAssemblyVersion).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
