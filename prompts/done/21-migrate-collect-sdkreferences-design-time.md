# Prompt: Migrate CollectSDKReferencesDesignTime for Multithreading
...
## Task File
`src/Tasks/Microsoft.NET.Build.Tasks/CollectSDKReferencesDesignTime.cs`

## What This Task Does
Collects SDK references for design-time builds.
...
- **Attribute check**: `typeof(CollectSDKReferencesDesignTime).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(CollectSDKReferencesDesignTime).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
