# Prompt: Migrate GetPackageDirectory for Multithreading
...
## What This Task Does
Gets the directory path for a NuGet package. May use file system checks.
...
- **Attribute check**: `typeof(GetPackageDirectory).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(GetPackageDirectory).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
