# Prompt: Migrate GetDefaultPlatformTargetForNetFramework for Multithreading
...
## What This Task Does
Returns the default platform target for .NET Framework projects.
...
- **Attribute check**: `typeof(GetDefaultPlatformTargetForNetFramework).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(GetDefaultPlatformTargetForNetFramework).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
