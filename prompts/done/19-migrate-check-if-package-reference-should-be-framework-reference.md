# Prompt: Migrate CheckIfPackageReferenceShouldBeFrameworkReference for Multithreading
...
## Task File
`src/Tasks/Microsoft.NET.Build.Tasks/CheckIfPackageReferenceShouldBeFrameworkReference.cs`

## What This Task Does
Suggests converting PackageReferences to FrameworkReferences where appropriate.
...
- **Attribute check**: `typeof(CheckIfPackageReferenceShouldBeFrameworkReference).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(CheckIfPackageReferenceShouldBeFrameworkReference).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
