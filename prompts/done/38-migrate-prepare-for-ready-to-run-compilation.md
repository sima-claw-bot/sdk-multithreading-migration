# Prompt: Migrate PrepareForReadyToRunCompilation for Multithreading
...
## What This Task Does
Prepares inputs for ReadyToRun (R2R) compilation. May set up file paths for compilation.
...
- **Attribute check**: `typeof(PrepareForReadyToRunCompilation).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(PrepareForReadyToRunCompilation).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
