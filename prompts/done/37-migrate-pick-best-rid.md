# Prompt: Migrate PickBestRid for Multithreading
...
## What This Task Does
Selects the best RuntimeIdentifier from available options using the runtime graph.
...
- **Attribute check**: `typeof(PickBestRid).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(PickBestRid).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
