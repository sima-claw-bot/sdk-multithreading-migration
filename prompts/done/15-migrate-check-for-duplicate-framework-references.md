# Prompt: Migrate CheckForDuplicateFrameworkReferences for Multithreading
...
## Task File
`src/Tasks/Microsoft.NET.Build.Tasks/CheckForDuplicateFrameworkReferences.cs`

## What This Task Does
Checks for duplicate FrameworkReference items and reports errors. Pure item inspection task.
...
- **Attribute check**: `typeof(CheckForDuplicateFrameworkReferences).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(CheckForDuplicateFrameworkReferences).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
