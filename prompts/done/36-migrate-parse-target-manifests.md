# Prompt: Migrate ParseTargetManifests for Multithreading
...
## What This Task Does
Parses runtime store target manifest files.
...
- **Attribute check**: `typeof(ParseTargetManifests).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(ParseTargetManifests).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
