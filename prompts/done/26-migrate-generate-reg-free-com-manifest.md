# Prompt: Migrate GenerateRegFreeComManifest for Multithreading
...
## What This Task Does
Generates registration-free COM manifest files. Likely writes XML to disk.
...
- **Attribute check**: `typeof(GenerateRegFreeComManifest).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(GenerateRegFreeComManifest).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
