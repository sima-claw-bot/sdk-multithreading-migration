# Prompt: Migrate GetNuGetShortFolderName for Multithreading
...
## Task File
`src/Tasks/Microsoft.NET.Build.Tasks/GetNuGetShortFolderName.cs`

## What This Task Does
Converts target framework to NuGet short folder name (e.g., net6.0).
...
- **Attribute check**: `typeof(GetNuGetShortFolderName).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(GetNuGetShortFolderName).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
