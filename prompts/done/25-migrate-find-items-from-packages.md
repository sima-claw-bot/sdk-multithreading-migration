# Prompt: Migrate FindItemsFromPackages for Multithreading
...
## Task File
`src/Tasks/Microsoft.NET.Build.Tasks/FindItemsFromPackages.cs`

## What This Task Does
Finds items from NuGet packages matching specified criteria.
...
- **Attribute check**: `typeof(FindItemsFromPackages).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(FindItemsFromPackages).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
