# Prompt: Migrate GetEmbeddedApphostPaths for Multithreading
...
## Task File
`src/Tasks/Microsoft.NET.Build.Tasks/GetEmbeddedApphostPaths.cs`

## What This Task Does
Gets paths to embedded apphost templates. May reference file paths.
...
- **Attribute check**: `typeof(GetEmbeddedApphostPaths).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(GetEmbeddedApphostPaths).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
