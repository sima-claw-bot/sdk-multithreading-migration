# Prompt: Migrate GetPackagesToPrune for Multithreading
...
## What This Task Does
Determines which packages should be pruned from the dependency graph.
...
- **Attribute check**: `typeof(GetPackagesToPrune).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(GetPackagesToPrune).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
