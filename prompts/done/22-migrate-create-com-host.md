# Prompt: Migrate CreateComHost for Multithreading
...
## What This Task Does
Creates a COM host (comhost) for COM server support. Similar to CreateAppHost - likely uses file paths for host binary operations.
...
- **Attribute check**: `typeof(CreateComHost).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(CreateComHost).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
