# Prompt: Migrate ShowMissingWorkloads for Multithreading
...
## What This Task Does
Shows messages about missing .NET workloads.
...
- **Attribute check**: `typeof(ShowMissingWorkloads).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(ShowMissingWorkloads).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
