# Prompt: Migrate RunReadyToRunCompiler for Multithreading
...
## What This Task Does
Runs the ReadyToRun compiler (crossgen/crossgen2). Extends ToolTask. Uses File.Exists, Directory.CreateDirectory, and Path operations extensively.
...
- **Attribute check**: `typeof(RunReadyToRunCompiler).Should().BeDecoratedWith<MSBuildMultiThreadableTaskAttribute>()`
- **If interface-based**: `typeof(RunReadyToRunCompiler).Should().BeAssignableTo<IMultiThreadableTask>()` + path resolution test using TaskEnvironment with a project dir different from CWD
