    ...
    public class ProjectFileAnalyzer : Microsoft.Build.Utilities.Task, IMultiThreadableTask
    {
        ...
        public override bool Execute()
        {
            try
            {
                ...
                string projectDir = Path.GetDirectoryName(absolutePath) ?? TaskEnvironment.ProjectDirectory;
                ...
                foreach (string refPath in projectRefs)
                {
                    ...
                    string refName = Path.GetFileNameWithoutExtension(resolvedPath);
        ...
        private List<ReferenceInfo> ResolveTransitiveReferences(List<string> directRefs)
        {
            ...
            foreach (string parentRef in directRefs)
            {
                ...
                try
                {
                    ...
                    {
                        // BUG: uses Path.Combine + Path.GetFullPath instead of TaskEnvironment.GetAbsolutePath()
                        ...
                        {
                            string refName = Path.GetFileNameWithoutExtension(resolvedTransitive);
